...
class Subtokenizer(object):
    ...
    def encode(self, raw_string, add_eos=False, max_length=2000):
        ...
        _the_rest_length = max_length - len(ret) - 1 # - length of ret and the final EOS

        ret = np.array(ret)
        ret = np.pad(ret, (0, _the_rest_length), constant_values=0)
        if add_eos:
            ret = np.pad(ret, (0, 1), constant_values=1)
  ...
  """
    ...
    for filepath in files:
        ...
        with open(filepath, mode="r") as reader:
            ...
            # I use Os.path to inplace the `reader.size`
            lines_to_skip = int(os.path.getsize(filepath) / (file_byte_budget * 2))
...
def _generate_subtokens_with_target_vocab_size(
    ...
    if min_count is not None:
        ...
        print("Using min_count=%d to generate vocab with target size %d" %(min_count, target_size))
    ...
    def bisect(min_val, max_val):
        ...
        #                 (cur_count, val))
        print("Binary search: min_count=%d resulted in %d tokens" %(cur_count, val))
